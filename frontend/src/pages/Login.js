...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 font-sans relative overflow-hidden">
      ...
      <div className="bg-transparent relative z-10 w-full max-w-[450px]">
        
        {/* Toggle Switch */}
        <div className="flex p-1 bg-white/10 backdrop-blur-md rounded-xl mb-4 border border-white/20">
          <button
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'login' ? (
              ...
              <StyledWrapper>
                <form className="form" onSubmit={handleLogin}>
                  ...
                  <div className="inputForm">
                    ...
                    />
                    <div 
                      style={{ cursor: 'pointer', paddingRight: '10px', display: 'flex' }}
                      onClick={() => setShowPwd(!showPwd)}
                    >
                      {showPwd ? <EyeOff size={18} color="#666" /> : <Eye size={18} color="#666" />}
                    </div>
                  </div>
                  <div className="flex-row">
                    ...
                    <span className="span" onClick={() => toast('Forgot password? Coming Soon! 🚀', { icon: '🔜' })}>Forgot password?</span>
                  </div>
                  <button type="submit" className="button-submit" disabled={loading}>
                    {loading ? 'Signing In...' : 'Sign In'}
                  </button>
                  <p className="p">Don't have an account? <span className="span" onClick={() => toast('Sign Up is Coming Soon! 🚀', { icon: '🔜' })}>Sign Up</span></p>
                  <p className="p line">Or With</p>
                  <div className="flex-row">
                    <button type="button" className="btn google" onClick={() => toast('Google Login Coming Soon! 🚀', { icon: '🔜' })}>
                    ...
                    <button type="button" className="btn apple" onClick={() => toast('Apple Login Coming Soon! 🚀', { icon: '🔜' })}>
            ...
            <motion.div
              ...
              exit={{ opacity: 0, x: -20 }}
              transition={{ duration: 0.2 }}
              className="flex flex-col items-center justify-center py-4 bg-white rounded-3xl p-8 shadow-2xl"
            >
              <img 
                src="/zust-eat.png" 
                alt="Zest Eat Logo" 
